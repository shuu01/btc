...
class App(object):
    ...
    def show_total_balance(self, data=None):
    ...
    def set_menu(self):
        ...
        # Add callbacks
        item_total_balance.connect_object("activate", self.show_total_balance, "BTC")
        item_exit.connect_object("activate", self.close, "Close app")
    ...
    def set_ordermenu(self):
        
        #for widget in self.ordermenu.get_children():
        #    self.ordermenu.remove(widget)
        ...
        for client in self.clients:
            ...
            #child.modify_font(Pango.FontDescription("Dejavu Sans Mono 24"))
            menu.append(menu_item)
            menu_item.connect_object("activate", self.open_url, url)
            menu_item.show()
            ...
            if orders:
                ...
                for order in orders:
                    ...
                    if side == 'buy':
                        color.parse('#0066AA')
                    color.to_string()
                    menu_item.override_color(gtk.StateFlags.NORMAL, color)
                    menu.add(menu_item)
                    menu_item.show()
            ...
            if history:
                ...
                for order_id, order in history.items():
                    ...
                    line = "%s %s: %s for %s (%s)" % (symbol, side, quantity, price, cur_price)
                    menu_item = gtk.ImageMenuItem(line)
                    ...
                    menu_item.modify_fg(gtk.StateFlags.NORMAL, color)
                    menu_item.connect("activate", self.confirm_order, {'menu': menu, 'table': table, 'id': order_id, 'exchange': name})
                    menu.append(menu_item)
                    menu_item.show()
    ...
    def menu_left(self, event_button, event_time, data=None):
        ...
        self.ordermenu.popup(None, None, None, self.statusicon, event_button, event_time)
    ...
    def confirm_order(self, widget, data=None):
    ...
    def update(self):
        ...
        for client in self.clients:
            ...
            #self.set_balance(exchange)
            #self.set_active_orders(exchange)
            symbols = []
